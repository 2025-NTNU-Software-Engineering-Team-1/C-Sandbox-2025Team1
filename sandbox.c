...
int main (int argc , char **argv)
{
    if (argc != 12 + 1) {
        ...
        fprintf (stderr , "Usage: %s lang_id compile file_stdin file_stdout file_stderr time_limit memory_limit large_stack output_limit process_limit allow_network file_result\n" , argv [0]);
        return 1;
    }
    ...
    long lang_id = strtol (argv [1] , NULL , 10) ,
        ...
        allow_network = strtol (argv [11] , NULL , 10);
    ...
    if (pid > 0) {
        ...
        if (!compile) {
            if (lang_id == 0 || lang_id == 1)
                c_cpp_rules (program , 0 , (bool)allow_network);
            if (lang_id == 2)
                general_rules (program , (bool)allow_network);
        }
